...
#if defined(NDEBUG)
# error "Zeronecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x0f5f71117ee77e9067fd5ceeb21aea0e0e1c92212b5538a4441d2454fbc6c633");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zeronecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Zeronecoin Signed Message:\n";
...
{
    // Zeronecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Zeronecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Zeronecoin: 5 minutes
static const int64 nTargetSpacing = 1 * 25; // Zeronecoin: 25 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zeronecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zeronecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0f5f71117ee77e9067fd5ceeb21aea0e0e1c92212b5538a4441d2454fbc6c633");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ZeroneCoin 1.0";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("694184710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515679240;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3067170;
        ...
        assert(block.hashMerkleRoot == uint256("0x2c4f5c8b0999ed99e1f78514ebc3ebc5d9dcb58e5739550d934406f3fe2605e6"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Zeronecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZeronecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZeronecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZeronecoinMiner(CWallet *pwallet)
{
    printf("ZeronecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("zeronecoin-miner");
    ...
    try { loop {
        ...
        printf("Running ZeronecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZeronecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZeronecoinMiner, pwallet));
}
